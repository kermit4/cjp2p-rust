...
struct PeerInfo {
    ...
    promoted_content: Option<PromotedContent>,
    ...
    fn probe(&mut self) -> () {
        for sa in self.best_peers(10, 3) {
            ...
            if let Some(last_viewed) = &self.last_viewed {
                message_out.push(Message::LastViewed(last_viewed.clone()));
            }
            if let Some(promoted_content) = &self.promoted_content {
                message_out.push(Message::PromotedContent(promoted_content.clone()));
            }
    ...
    fn handle_messages(
        ...
        for message_in_enum in messages {
            message_out.append(&mut match message_in_enum {
                ...
                Message::Search(t) => t.receive(self, src, might_be_ip_spoofing),
                Message::SearchResult(t) => t.receive(self, src),
