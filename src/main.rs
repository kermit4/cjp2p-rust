...
struct PeerInfo {
    ...
    anti_ip_spoofing_cookie_i_expect: String, // to save memory, these could be generated by hashing their ip:port with a fixed 90 bits of entropy, ideally stored between runs
    #[serde_as(as = "Option<Base64>")]
    ed25519: Option<Vec<u8>>,
    promoted_content: Option<YouSouldSeeThis>,
    last_viewed: Option<IJustSawThis>,
}
impl PeerInfo {
    fn new() -> Self {
        return Self {
    ...
    fn check_key(&self, messages: &Vec<Message>, src: SocketAddr) -> bool {
        for message_in in messages {
            if let Message::AlwaysReturned(m) = message_in {
                let correct_key = &self.peer_map[&src].anti_ip_spoofing_cookie_i_expect;
                return *correct_key != m.cookie;
            }
        }
        return true;
    }
    fn please_always_return(&self, src: SocketAddr) -> Message {
        let correct_key = self.peer_map[&src]
            .anti_ip_spoofing_cookie_i_expect
            .to_owned();
    ...
    fn probe(&mut self) -> () {
        for sa in self.best_peers(10, 3) {
            ...
            if let Some(last_viewed) = &self.last_viewed {
                message_out.push(Message::IJustSawThis(last_viewed.clone()));
            }
            if let Some(promoted_content) = &self.promoted_content {
                message_out.push(Message::YouSouldSeeThis(promoted_content.clone()));
            }
    ...
    fn handle_messages(
        ...
        for message_in_enum in messages {
            message_out.append(&mut match message_in_enum {
                ...
                Message::List(t) => t.receive(self, src, might_be_ip_spoofing),
                Message::ListResult(t) => t.receive(self, src),
        ...
        if read_fds.contains(stdin.as_fd()) {
            ...
            if line.len() > 1 {
                ...
                if sscanf!(line.as_str(), "/get {}",arg).is_ok() {
                    ps.last_viewed = Some(IJustSawThis {
                        id: arg.to_owned(),
                    ...
                    ps.promoted_content = Some(YouSouldSeeThis {
                        id: arg.to_owned(),
                        length: File::open(&arg).unwrap().metadata().unwrap().len(),
                    });
...
impl List {
    ...
    fn receive(
        ...
        return ListResult::new(might_be_ip_spoofing);
    }
}
#[derive(Serialize, Deserialize, Debug)]
struct ListResult {
...
impl ListResult {
    fn new(might_be_ip_spoofing: bool) -> Vec<Message> {
        ...
        let message_out = vec![
            Message::ListResult(Self { results: results }),
        ];
        return message_out;
    }
    fn receive(&self, ps: &mut PeerState, src: SocketAddr) -> Vec<Message> {
